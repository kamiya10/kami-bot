...
const { EmbedBuilder, SlashCommandBuilder, SlashCommandStringOption, TimestampStyles, codeBlock, time } = require("@discordjs/builders");
 ...
 * @returns {KamiCommand}
 */
const colorCommand = (client) => new KamiCommand({
  dev      : true,
  filePath : __filename,
  ...
  async execute(interaction) {
    try {
      ...
      const embed = new EmbedBuilder()
        .setAuthor({
          name    : $t("header:color", { lng: interaction.locale, 0: interaction.user.displayName }),
      ...
      embed
        ...
        .addFields(...[
          {
            name  : "Color Name",
            value : `${name || "*`None`*"}${name != namePossible ? ` (around ${namePossible})` : ""}`,
          },
          {
            name   : "hex",
          ...
          {
            name   : "decimal",
            value  : `\`\`\`js\n${isOpaque ? Number(`0x${hex}`) : Number(`0x${hex8}`)}\`\`\`\n\`\`\`\nDecimal(${isOpaque ? Number(`0x${hex}`) : Number(`0x${hex8}`)})\`\`\``,
            inline : true,
          },
          {
            name  : isOpaque ? "rgb" : "rgba",
            value : `🟥 ${rgb.r} (${rgbPercentage.r})　🟩 ${rgb.g} (${rgbPercentage.g})　🟦 ${rgb.b} (${rgbPercentage.b})\n\`\`\`css\n${isOpaque ? "rgb" : "rgba"}(${rgb.r}, ${rgb.g}, ${rgb.b}${!isOpaque ? `, ${rgb.a}` : ""})\`\`\`\n\`\`\`css\nrgb(${rgb.r} ${rgb.g} ${rgb.b}${!isOpaque ? ` / ${rgb.a}` : ""})\`\`\``,
          },
          {
            name   : "hsv",
            value  : `🏳️‍🌈 ${hsv[0]}°　<:hsv_s:1184500143567945778> ${hsv[1]}%　<:hsv_v:1184500676227764266> ${hsv[2]}%\n\`\`\`css\n${isOpaque ? "hsv" : "hsva"}(${hsv[0]}deg, ${hsv[1]}%, ${hsv[2]}%${!isOpaque ? `, ${rgb.a}` : ""})\`\`\`\n\`\`\`css\nhsv(${hsv[0]}deg ${hsv[1]}% ${hsv[2]}%${!isOpaque ? ` / ${rgb.a}` : ""})\`\`\``,
            inline : true,
          },
          {
            name   : "hsl",
            value  : `🏳️‍🌈 ${hsl[0]}°　<:hsl_s:1184501512999800843> ${hsl[1]}%　<:hsl_l:1184500903764566146> ${hsl[2]}%\n\`\`\`css\n${isOpaque ? "hsl" : "hsla"}(${hsl[0]}deg, ${hsl[1]}%, ${hsl[2]}%${!isOpaque ? `, ${rgb.a}` : ""})\`\`\`\n\`\`\`css\nhsl(${hsl[0]}deg ${hsl[1]}% ${hsl[2]}%${!isOpaque ? ` / ${rgb.a}` : ""})\`\`\``,
            inline : true,
          },
          {
            name  : "hwb",
            value : `🏳️‍🌈 ${hwb[0]}°　⬜ ${hwb[1]}%　⬛ ${hwb[2]}%\n\`\`\`css\n${isOpaque ? "hwb" : "hwba"}(${hwb[0]}deg, ${hwb[1]}%, ${hwb[2]}%${!isOpaque ? `, ${rgb.a}` : ""})\`\`\`\n\`\`\`css\nhwb(${hwb[0]}deg ${hwb[1]}% ${hwb[2]}%${!isOpaque ? ` / ${rgb.a}` : ""})\`\`\``,
          },
          {
            name   : "xyz",
            value  : `🇽 ${xyz[0]}　🇾 ${cmyk[1]}　🇿 ${xyz[2]}\n\`\`\`css\n${isOpaque ? "xyz" : "xyza"}(${xyz[0]}, ${xyz[1]}, ${xyz[2]}${!isOpaque ? `, ${rgb.a}` : ""})\`\`\``,
            inline : true,
          },
          {
            name   : "yiq",
            value  : `🇾 ${yiq[0]}　🇮 ${yiq[1]}　🇶 ${yiq[2]}\n\`\`\`css\n${isOpaque ? "yiq" : "yiqa"}(${yiq[0]}, ${yiq[1]}, ${yiq[2]}${!isOpaque ? `, ${rgb.a}` : ""})\`\`\``,
            inline : true,
          },
          {
            name  : "Android ARGB (android.graphics.Color)",
            value : `\`\`\`js\n${Number(`0x${hex8.slice(6)}${hex}`)}\`\`\`\n\`\`\`js\n0x${hex8.slice(6).toUpperCase()}${hex.toUpperCase()}\`\`\`\n\`\`\`js\n0x${hex8.slice(6)}${hex}\`\`\``,
          },
          {
            name  : "cmy",
            value : `<:cmyk_c:1184522220995874847> ${cmy[0]} (${Math.round(cmy[0] * 100)}%)　<:cmyk_m:1184522450306879548> ${cmy[1]} (${Math.round(cmy[1] * 100)}%)　🟨 ${cmy[2]} (${Math.round(cmy[2] * 100)}%)\n\`\`\`css\ncmy(${cmy[0]}%, ${cmy[1]}%, ${cmy[2]}%)\`\`\`\n\`\`\`css\ncmy(${cmy[0]}% ${cmy[1]}% ${cmy[2]}%)\`\`\``,
          },
          {
            name  : "cmyk",
            value : `<:cmyk_c:1184522220995874847> ${cmyk[0] / 100} (${cmyk[0]}%)　<:cmyk_m:1184522450306879548> ${cmyk[1] / 100} (${cmyk[1]}%)　🟨 ${cmyk[2] * 100} (${cmyk[2]}%)　⬛ ${cmyk[3] / 100} (${cmyk[3]}%)\n\`\`\`css\ncmyk(${cmyk[0]}%, ${cmyk[1]}%, ${cmyk[2]}%, ${cmyk[3]}%)\`\`\`\n\`\`\`css\ncmyk(${cmyk[0]}% ${cmyk[1]}% ${cmyk[2]}% ${cmyk[3]}%)\`\`\``,
          },
          {
            name  : "ansi",
            value : `ansi16 \`${ansi16}\`\nansi256 \`${ansi256}\``,
