...
function formatEarthquake(Earthquake, style = 0) {
  ...
  const depthI = depth.sort((a, b) => a - b).indexOf(Earthquake.EarthquakeInfo.FocalDepth);
  const magnitudeI = ~~Earthquake.EarthquakeInfo.EarthquakeMagnitude.MagnitudeValue;
  ...
  switch (style) {
    ...
    case 4: {
      ...
      const embed = new EmbedBuilder()
        ...
        .setFields(
          ...[
            { name: "編號", value: `${Earthquake.EarthquakeNo % 1000 == 0 ? "無（小區域有感地震）" : Earthquake.EarthquakeNo}`, inline: true },
            { name: "時間", value: `<t:${~~(time.getTime() / 1000)}:D><t:${~~(time.getTime() / 1000)}:T>`, inline: true },
            { name: "深度", value: `${depthE[depthI]}**${Earthquake.EarthquakeInfo.FocalDepth}** 公里 \`(${depthTW[depthI]})\``, inline: true },
            { name: "規模", value: `${magnitudeE[magnitudeI]}芮氏 **${Earthquake.EarthquakeInfo.EarthquakeMagnitude.MagnitudeValue}** \`(${magnitudeTW[magnitudeI]})\``, inline: true },
      ...
      const embed = new EmbedBuilder()
        ...
        .setFields(
          ...[
            ...
            { name: "震央經緯", value: `北緯 **${Earthquake.EarthquakeInfo.Epicenter.EpicenterLatitude}** 度\n東經 **${Earthquake.EarthquakeInfo.Epicenter.EpicenterLongitude}**   度`, inline: true },
            { name: "深度", value: `${depthE[depthI]} **${Earthquake.EarthquakeInfo.FocalDepth}** 公里\n　 ${depthTW[depthI]}`, inline: true },
