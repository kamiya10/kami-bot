const { Colors, EmbedBuilder, SlashCommandAttachmentOption, SlashCommandBooleanOption, SlashCommandBuilder, SlashCommandUserOption } = require("discord.js");
  ...
  async execute(interaction) {
    ...
    const embeds = [
      new EmbedBuilder()
        ...
        .setAuthor({ name: "SauceNAO", iconURL: "https://upload.cc/i1/2022/12/25/VYQN15.png", url: "https://saucenao.com" })
    ...
    for (const result of results)
      try {
        ...
        description += "ext_urls" in result.raw.data
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor(tinycolor.mix(tinycolor.mix(white, green, result.similarity), red, 100 - result.similarity).toHex())
          .setThumbnail(result.thumbnail)
          .setAuthor({ name: `${result.index}: ${result.site} (${result.similarity}%)` })
