const { ActionRowBuilder, AutocompleteInteraction, ButtonBuilder, ButtonStyle, Collection, Colors, ComponentType, EmbedBuilder, SlashCommandBuilder, SlashCommandIntegerOption, SlashCommandNumberOption, SlashCommandStringOption, SlashCommandSubcommandBuilder, SlashCommandSubcommandGroupBuilder, TimestampStyles, time } = require("discord.js");
const { TrainDirectionName, TrainType, TrainTypeName } = require("../../API/tdx");
...
const facilityIcon = (trainInfo) => {
  ...
  if (trainInfo.Note.includes("跨日")) str.push("<:TRA_crossday:1135723683684560987> 跨日列車");

  if (trainInfo.Note.includes("自由座")) str.push("<:TRA_freeseat:1135590263620907060> 自由座");

  if (trainInfo.Note.includes("騰雲座艙")) str.push("<:TRA_bussiness:1135585152706289734> 騰雲座艙");

  if (trainInfo.TrainTypeCode == TrainType.PuyumaExpress) str.push("<:TRA_table:1135584917921738772> 桌型座");

  if (trainInfo.DiningFlag) str.push("<:TRA_lunchbox:1135584866843496649> 訂便當服務");

  if (trainInfo.DailyFlag || trainInfo.Note.includes("每日行駛")) str.push("<:TRA_everyday:1135580865276153896> 每日列車");

  if (trainInfo.Note.includes("親子車廂")) str.push("<:TRA_parenting:1135581088488624158> 親子車廂");

  if (trainInfo.BikeFlag) str.push("<:TRA_bicycle:1135585523109478401> 可載運自行車");

  if (trainInfo.BreastFeedFlag) str.push("<:TRA_nursing:1135581420895604919> 哺(集)乳室");

  if (trainInfo.WheelChairFlag) str.push("<:TRA_wheelchair:1135581477409656914> 身障旅客專用座位");
...
const handleTrainNumberAutocomplete = async (interaction) => {
  ...
  const choice = timetables.TrainTimetables
    .map(v => ({ name: `${v.TrainInfo.TrainNo}次 ${v.TrainInfo.TripHeadSign} ${TrainTypeName[v.TrainInfo.TrainTypeCode]}`, value: v.TrainInfo.TrainNo }))
...
const handleTrackDepartureAutocomplete = async (interaction) => {
  const timetables = cache.get("trainTimetable");
  const timetable = timetables.TrainTimetables.find(v => v.TrainInfo.TrainNo == interaction.options.getString("number"));

  if (!timetable) return;

  const focused = interaction.options.getFocused(true);
  const choice = timetable.StopTimes
    .map(v => ({ name: `${v.StopSequence}. ${v.StationName.Zh_tw} ${v.StationID}`, value: v.StationID }))
...
async (interaction) => {
  ...
  const choice = timetable.StopTimes
    .map(v => ({ name: `${v.StopSequence}. ${v.StationName.Zh_tw} ${v.StationID}`, value: v.StationID, sequence: v.StopSequence }))
    .reduce((acc, v, i, a) => (i > a.findIndex(val => val.value == departure) && acc.push(v), acc), [])
    .filter(v => !focused.value || v.name.includes(focused.value))
    .slice(0, 25);
  await interaction.respond(choice);
};
  ...
  switch (interaction.options.getSubcommand()) {
    ...
    case "number": {
      ...
      const pages = timetable.StopTimes.reduce((acc, item, index, arr) => {
        ...
        return acc;
      }, []).map(page => page.map((v) => `<:TRA:1134700741479628900> ${v.isOriginatingStation ? "**始發站**" : `\`${v.ArrivalTime}\``} ${v.StationName.Zh_tw} ${v.isTerminalStation ? "**終點站**" : `\`${v.DepartureTime}\``}`).join("\n"));

      const baseEmbed = new EmbedBuilder()
        .setColor(Colors.Blue)
        .setAuthor({ name: `${timetable.TrainInfo.TrainNo} ${TrainTypeName[timetable.TrainInfo.TrainTypeCode]}` })
        .setThumbnail(trainIcon(timetable.TrainInfo.TrainTypeCode, timetable.TrainInfo.TrainNo))
        .setTitle(`${timetable.TrainInfo.StartingStationName.Zh_tw} → ${timetable.TrainInfo.EndingStationName.Zh_tw}`)
      ...
      const paginator = (index) => new ActionRowBuilder()
        .setComponents([
          new ButtonBuilder()
      ...
      collector.on("collect", async (ci) => {
        switch (ci.customId) {
          case "first-page": pageIndex = 0; break;
          case "prev-page": pageIndex--; break;
          case "next-page": pageIndex++; break;
          case "last-page": pageIndex = pages.length - 1; break;
...
module.exports = {
  ...
  async autoComplete(interaction) {
    ...
    switch (focused.name) {
      case "number": await handleTrainNumberAutocomplete(interaction); break;
      case "departure": await handleTrackDepartureAutocomplete(interaction); break;
      case "arrival": await handleTrackArrivalAutocomplete(interaction); break;
