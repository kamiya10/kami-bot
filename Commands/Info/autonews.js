...
module.exports = {
  ...
  async execute(interaction) {
    const embed = new EmbedBuilder()
      .setAuthor({
        name: `自動公告發佈頻道 | ${interaction.guild.name}`,
        iconURL: interaction.guild.iconURL({ dynamic: true }),
      })
      .setColor(Colors.Blue)
      .setFooter({
        text: "在頻道的主題加上 [AutoNews] 就可以把頻道設為自動公告發佈頻道",
      })
      .setTimestamp();

    const autonews = interaction.guild.channels.cache
      .filter((channel) => channel?.topic?.includes("[AutoNews]"))
      .map((channel) => {
        const permissions = [];
