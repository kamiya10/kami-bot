...
const cwa_Forecast = new (require("../../API/cwa_forecast"))(
  process.env.CWA_TOKEN,
);
    ...
    if (data)
      for (const cyclone of data.tropicalCyclones.tropicalCyclone) {
        ...
        const mainEmbed = new EmbedBuilder()
          ...
          .setTitle(
            `${getCycloneLevel(current.maxWindSpeed, current.coordinate)}：${cyclone.typhoonName} ${cyclone.cwaTyphoonName}`,
          )
          .setURL("https://www.cwa.gov.tw/V8/C/P/Typhoon/TY_NEWS.html")
          .setDescription(
            `${time(new Date(current.fixTime), TimestampStyles.ShortDateTime)} 的中心位置在${coord.latitude == 0 ? "" : coord.latitude > 0 ? "北緯" : "南緯"} ${coord.latitude} 度，${coord.latitude == 0 ? "" : coord.latitude > 0 ? "東經" : "西經"} ${coord.longitude} 度，以每小時 ${cyclone.forecastData.fix[0].movingSpeed} 公里速度，向${Bearings[cyclone.forecastData.fix[0].movingDirection]}進行。中心氣壓 ${current.pressure} 百帕，近中心最大風速每秒 ${current.maxWindSpeed} 公尺，瞬間最大陣風每秒 ${current.maxGustSpeed} 公尺，七級風暴風半徑 ${current.circleOf15Ms.radius} 公里${current.circleOf25Ms ? `，十級風暴風半徑 ${current.circleOf25Ms.radius} 公里。` : "。"}`,
          )
          .setImage(
            `https://www.cwa.gov.tw/Data/typhoon/TY_NEWS/PTA_${cTimeId}-${lastForecast.tau}_${cyclone.typhoonName}_zhtw.png`,
          );

        const mainEmbedRadiirImage = new EmbedBuilder()
          .setURL("https://www.cwa.gov.tw/V8/C/P/Typhoon/TY_NEWS.html")
          .setImage(
            `https://www.cwa.gov.tw/Data/typhoon/TY_NEWS/WSP-MAP_${cTimeId}_${cyclone.typhoonName}_zhtw.png`,
          );
        ...
        const historyEmbed = new EmbedBuilder()
          ...
          .setTitle(
            `${getCycloneLevel(current.maxWindSpeed, current.coordinate)}：${cyclone.typhoonName} ${cyclone.cwaTyphoonName} - 歷史路徑`,
          )
          .setURL("https://www.cwa.gov.tw/V8/C/P/Typhoon/TY_NEWS.html");

        const forecastEmbed = new EmbedBuilder(historyEmbed.data).setTitle(
          `${getCycloneLevel(current.maxWindSpeed, current.coordinate)}：${cyclone.typhoonName} ${cyclone.cwaTyphoonName} - 預測路徑`,
        );
        ...
        for (const history of cyclone.analysisData.fix) {
          ...
          if (
            new Date(Date.now()).getDate() != history.fixTime.getDate() &&
            !(
              ~~(
                (new Date(Date.now()).getTime() - history.fixTime.getTime()) /
                86400_000
              ) >= 2
                ? [8]
                : [8, 20]
        ...
        for (const history of cyclone.analysisData.fix) {
          ...
          str.push(
            `${bars[cur][1]} 　位置　　　　　 │ ${coordinate.latitude == 0 ? "" : coordinate.latitude > 0 ? "東經" : "西經"} **${coordinate.longitude}** 度，${coordinate.latitude == 0 ? "" : coordinate.latitude > 0 ? "北緯" : "南緯"} **${coordinate.latitude}** 度`,
          );
          str.push(
            `${bars[cur][2]} 　中心氣壓　　　 │ **${history.pressure} 百帕**`,
          );
          str.push(
        ...
        for (const forecast of cyclone.forecastData.fix) {
          forecast.fixTime = new Date(
            new Date(forecast.initTime).getTime() +
              +forecast.tau * 60 * 60 * 1000,
          );
