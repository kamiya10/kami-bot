...
const mySauce = new SauceNAO("05e07f80d5ad365cb7c6c58568c21345f5c145d3");
        ...
        if (args.length) {
            if (args[0].startsWith("https://discord.com/channels/")) {
                ...
                mySauce(args[0]).then((response) => {
                    ...
                    console.dir(response.json);
                    console.log(response.json.results[0]);
                    if (response.json.results[0].header.index_name.match(/(nhentai)/gi) && message.channel.nsfw === false) {
                        const saucenotfound = new Discord.MessageEmbed()
                            .setColor(message.member.displayHexColor)
