...
module.exports = {
  ...
  execute(client, message) {
    ...
    try {
      ...
      const eewchannels = Object.keys(GuildSetting)
        .filter((v) => GuildSetting[v].eew_channel != null)
        .map((v) => [GuildSetting[v].eew_channel, GuildSetting[v].eew_mention]);
      ...
      event.data.forEach((data) => {
        ...
        for (const city in pos) {
          ...
          for (const town in pos[city]) {
            ...
            expected[city][town].intString = intensityToString(
              expected[city][town].int,
            );
        ...
        const embed = new EmbedBuilder()
          ...
          .setFields(
            ...[
              {
                name: "規模",
                value: `${magnitudeE[magnitudeI]} 芮氏 **${data.magnitude}** \`(${magnitudeTW[magnitudeI]})\``,
                inline: true,
              },
              {
                name: "深度",
                value: `${depthE[depthI]} **${data.depth}** 公里 \`(${depthTW[depthI]})\``,
                inline: true,
              },
              {
                name: "發生時間",
                value: `<t:${~~(et.getTime() / 1000)}:T>（<t:${~~(et.getTime() / 1000)}:R>）`,
                inline: true,
              },
              {
                name: "最大震度",
                value: `${max.location} **${max.intString}**`,
                inline: true,
              },
              {
                name: "最靠近震央",
                value: `${nearest.location} **${nearest.intString}**`,
                inline: true,
              },
              {
                name: "震央位置",
              ...
              {
                name: "預估震度",
                value: `${max.int >= 7 ? "**> 🏚️ 此地震可能會造成災害，勿驚慌、趴下、掩護、穩住。**" : data.magnitude >= 5.5 ? "**> 🚸 本次搖晃可能較多地區有感，請小心自身周邊安全。**" : ""}\n${Object.keys(
                  maxAll,
                )
                  .map((k) => ({
                    text: `${k} **${maxAll[k].intString}**`,
                    pga: maxAll[k].pga,
                  }))
                  .sort((a, b) => b.pga - a.pga)
                  .map((v) => v.text)
                  .join("\n")}`,
              },
            ],
          ...
          .setFooter({
            text: `發布於 ${pt.getHours() < 10 ? "0" : ""}${pt.getHours()}:${pt.getMinutes() < 10 ? "0" : ""}${pt.getMinutes()}:${pt.getSeconds() < 10 ? "0" : ""}${pt.getSeconds()}`,
          })
          .setTimestamp();
          ...
          eewchannels.forEach(async (v) => {
            try {
              ...
              if (ch) {
                const sent = await ch
                  .send({
                    content: `⚠ 強震即時警報 ${v[1] ? ch.guild.roles.cache.get(v[1]) : ""}`,
                    embeds: [embed],
                    ...(v[1]
                      ? { allowedMentions: { roles: [v[1]] }, parse: ["roles"] }
                      : {}),
                  })
                  .catch((e) =>
                    logger.error(
                      `無法發送速報 #${client.channels.cache.get(v[0]).name} ${v[0]} ${e}`,
                    ),
                  );
...
const gov = {
  ...
  臺北市: [25.0375394312419, 121.56442807553141],
  桃園縣: [24.992903262814142, 121.3010570064923],
  臺南市: [22.99221533270023, 120.18502754419917],
  ...
  基隆市: [25.13178397075426, 121.74445819736491],
  ...
  金門縣: [24.43689307522158, 118.318704656769],
  澎湖縣: [23.570004104100342, 119.56638097986993],
  連江縣: [26.157798573764861, 119.95192319072953],
};

function calRelative(lon, lat) {
  distances = Object.keys(gov).map((k) =>
    caldistance({ lat: gov[k][0], lon: gov[k][1] }, { lat, lon }),
  );
...
function pgaToIntensity(pga) {
  return [0.8, 2.5, 8.0, 25, 80, 140, 250, 440, 800, pga]
    .sort((a, b) => a - b)
    .indexOf(pga);
}
