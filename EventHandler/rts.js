...
module.exports = {
  ...
  execute(client, data) {
    ...
    const rts_channels = Object.keys(GuildSetting)
      .filter((v) => GuildSetting[v].rts_channel != null)
      .map((v) => [
        GuildSetting[v].rts_channel,
        GuildSetting[v].rts_metion,
        GuildSetting[v].rts_alert,
      ]);

    const embed = new EmbedBuilder()
      .setAuthor({ name: "地震檢知" })
      .setFooter({
        text: "此為實驗性功能，即時資料由 ExpTech 探索科技提供。",
        iconURL: "https://upload.cc/i1/2023/01/16/mtKV7B.png",
      });
      ...
      if (Object.keys(data.list).length > 10) {
        ...
        embed.addFields({
          ...
          value: Object.keys(data.list)
            .map((uuid) => ({
              name: client.data.rts_stations.get(uuid).Loc,
              intensity: data.list[uuid],
            }))
            .sort((a, b) => b.intensity - a.intensity)
            .map(
              (area) =>
                `${area.name} ${intensesTW[area.intensity] ?? "\\◾誤報"}`,
            )
            .join("\n"),
        });
      }
      ...
      const timer = () => {
          ...
          for (const setting of rts_channels) {
            ...
            if (message) {
              ...
              if (channel instanceof TextChannel) {
                const sent = channel
                  .send({
                    content: `⚠ 地震檢知 ${setting[1] ? channel.guild.roles.cache.get(setting[1]) : ""}`,
                    embeds: [embed_cache[data.id].embed],
                    components: embed_cache.end
                      ? []
                      : [new ActionRowBuilder({ components: [button] })],
                    ...(setting[1]
                      ? {
                          allowedMentions: { roles: [setting[1]] },
                          parse: ["roles"],
                        }
                      : {}),
                  })
                ...
                sent.then((v) => {
                  ...
                  collector.on("collect", async (i) => {
                    ...
                    if (
                      embed_cache[data.id].felt.find(
                        (value) => value.id == i.user.id,
                      ) == undefined
                    ) {
                      embed_cache[data.id].felt.push({
                        id: i.user.id,
        ...
        if (embed_cache[data.id].end) {
          const max = Object.keys(embed_cache[data.id].maxints)
            .map((uuid) => ({
              name: client.data.rts_stations.get(uuid).Loc,
              intensity: embed_cache[data.id].maxints[uuid],
            }))
            ...
            .addFields(
              ...[
                ...
                },
                {
                  name: "⏹️ 結束時間",
                  value: time(
                    ~~(embed_cache[data.id].lastTimestamp / 1000),
                    TimestampStyles.ShortDateTime,
                  ),
                  inline: true,
                },
          ...
          const urlButton = new ButtonBuilder()
            ...
            .setEmoji("📊")
            .setURL(
              `https://exptech.com.tw/api/v1/file/trem-report.html?id=${data.report_id}`,
            );
            ...
            if (message instanceof Message)
              if (!embed_cache[data.id].cancelled) {
                ...
                if (embed_cache[data.id].felt.length) {
                  embed_cache[data.id].felt = embed_cache[data.id].felt.sort(
                  ...
                  if (ranking.length) {
                    ...
                    message
                      .reply({
                        embeds: [sendEmbed],
                        components: embed_cache[data.id].alert
                          ? [
                              new ActionRowBuilder({
                                components: [urlButton, historyButton],
                              }),
                            ]
                          : [],
                        allowedMentions: {
