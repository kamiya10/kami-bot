...
module.exports = {
  ...
  async execute(client, oldState, newState) {
    try {
      ...
      const GuildSettings = await client.database.GuildDatabase.get(
        newState.guild.id,
      );
      const UserSettings = await client.database.UserDatabase.get(
        newState.member.id,
      );
       ...
       */
      const perms = newState.guild.members.me.permissions.has(
      ...
      perms.concat(
        Array.from(channel.parent.permissionOverwrites.cache.values()),
      );
      ...
      const channelSetting = {
        ...
        parent: category,
        userLimit: +(UserSettings?.voice?.limit
          ? UserSettings.voice.limit
          : setting.channelSettings.limit),
        bitrate: +(UserSettings?.voice?.bitrate
          ? UserSettings.voice.bitrate
          : setting.channelSettings.bitrate),
        rtcRegion: UserSettings?.voice?.region
          ? UserSettings.voice.region
          : (setting.channelSettings.region ?? null),
        videoQualityMode: +(UserSettings?.voice?.quality
          ? UserSettings.voice.quality
          : (setting.channelSettings.quality ?? 1)),
      ...
      await newState.guild.channels
        ...
        .then(async (ch) => {
          await newState.setChannel(ch).catch((e) => void e);
          client.watchedChanels.set(ch.id, {
            master: newState.member.id,
            creator: setting.creator,
          });
          timestamps.set(newState.member.id, now);
          setTimeout(
            () => timestamps.delete(newState.member.id),
            cooldownAmount,
          );
        })
        .catch((e) => void e);
