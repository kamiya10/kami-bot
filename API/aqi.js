class AQI {
  ...
  async getSystemTime() {
    return (
      await (
        await fetch("https://airtw.moenv.gov.tw/ajax.aspx", {
          headers: {
  ...
  async getCountyNames(time) {
    ...
    if (res.ok) {
      ...
      this.countyNames = data.reduce(
        (acc, v) => ((acc[v.Name] = v.Value), acc),
        {},
      );
  ...
  async getSiteData(siteId, time = new Date(Date.now())) {
    ...
    if (res.ok) {
      const data = await res.json();
      return data.reduce(
        (acc, v) => ((acc[Object.keys(v)[0]] = v[Object.keys(v)[0]]), acc),
        {},
      );
    } else {
      throw new Error(res.status);
