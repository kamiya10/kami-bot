...
const requiredPermissions = [PermissionFlagsBits.ManageChannels, PermissionFlagsBits.MoveMembers] as const;
...
const handleSetupProgress =
   ...
   */
  async (interaction: ChatInputCommandInteraction<CacheType>, client: KamiClient, guildVoiceData: Record<string, GuildVoiceSettings>) => {
    const permissions = interaction.guild?.members.me?.permissions;
    const permissionEmbed = new EmbedBuilder();

    if (!permissions) {
      permissionEmbed.setColor(Colors.Red)
        .setDescription(`Insufficient Permissions. (Missing ${requiredPermissions.join(" ")})`);
      ...
      permissionEmbed.setColor(Colors.Red)
        .setDescription(`Insufficient Permissions. (Missing ${permissions.missing(requiredPermissions).join(" ")})`);
    ...
    if (lastCategory instanceof CategoryChannel) {
      const lastCategoryChildren = lastCategory.children.cache
        .sort((a, b) => ((a.position + 1) << (voiceChannelTypes.includes(a.type) ? 8 : 0)) - ((b.position + 2) << (voiceChannelTypes.includes(b.type) ? 8 : 0)));
  ...
  async execute(interaction) {
    try {
      ...
      if (interaction.options.getSubcommandGroup(false)) {
          ...
          case "name": {
            ...
            if (channel) {
              if (channel.id in guildVoiceData) {
                ...
                embed
                  .setColor(Colors.Green)
                  .setDescription(`The default channel creation name has been ${name ? `set to ${bold(inlineCode(name))} ${nameOverride ? "and will override user settings" : ""}` : `${bold("cleared")}`} for ${channel.toString()}.`);
              } else {
                embed
                  .setColor(Colors.Red)
          ...
          case "limit": {
            ...
            if (channel) {
              if (channel.id in guildVoiceData) {
                ...
                embed
                  .setColor(Colors.Green)
                  .setDescription(`The default channel creation user limit has been ${limit ? `set to ${bold(inlineCode(`${limit || "unlimited"}`))} ${limitOverride ? "and will override user settings" : ""}` : `${bold("cleared")}`} for ${channel.toString()}.`);
              } else {
                embed
                  .setColor(Colors.Red)
                  .setDescription("This channel is not a temporary voice channel creator.");
          ...
          case "bitrate": {
            ...
            if (channel) {
              if (channel.id in guildVoiceData) {
                ...
                embed
                  .setColor(Colors.Green)
                  .setDescription(`The default channel creation bitrate has been ${bitrate ? `set to ${bold(inlineCode(`${bitrate} kbps`))} ${bitrateOverride ? "and will override user settings" : ""}` : `${bold("cleared")}`} for ${channel.toString()}.`);
              } else {
                embed
                  .setColor(Colors.Red)
                  .setDescription("This channel is not a temporary voice channel creator.");
          ...
          case "region": {
            ...
            if (channel) {
              if (channel.id in guildVoiceData) {
                ...
                embed
                  .setColor(Colors.Green)
                  .setDescription(`The default channel creation region has been ${region ? `set to ${bold(inlineCode(region || "auto"))} ${regionOverride ? "and will override user settings" : ""}` : `${bold("cleared")}`} for ${channel.toString()}.`);
              } else {
                embed
                  .setColor(Colors.Red)
                  .setDescription("This channel is not a temporary voice channel creator.");
        ...
        const memberIsInTrackedVoiceChannel = interaction.member.voice.channel instanceof VoiceChannel && client.states.voice.has(interaction.member.voice.channel.id);
        ...
        switch (subcommand) {
          ...
          case "name": {
            ...
            } else if (interaction.member.voice.channel && client.states.voice.has(interaction.member.voice.channel.id)) {
