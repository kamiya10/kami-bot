...
import { ChannelType, Colors, EmbedBuilder, SlashCommandBooleanOption, SlashCommandBuilder, SlashCommandChannelOption, codeBlock } from "discord.js";
...
export const build = (client: KamiClient): KamiCommand => new KamiCommand({
  ...
  async execute(interaction) {
    try {
      ...
      if (channel) {
        client.database.guild(interaction.guild.id).earthquake.rts.channelId = channel.id;
        content += `已將推播頻道設為 **${channel.toString()}**`;
        if (autoDelete == undefined) {
          content += `。`;
        }
      } else {
        client.database.guild(interaction.guild.id).earthquake.rts.channelId = null;
        content += `已**清除**推播頻道`;
        if (autoDelete == undefined) {
          content += `。`;
        }
      ...
      if (autoDelete != undefined) {
        client.database.guild(interaction.guild.id).earthquake.rts.autoDelete = autoDelete;
      ...
      const embed = new EmbedBuilder()
        .setAuthor({
          name: $t("header:rts", { lng: interaction.locale, 0: interaction.guild.name }),
