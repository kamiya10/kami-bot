...
process.stdout.write(
  `${String.fromCharCode(27)}]0;Kami ${Kami.version}${String.fromCharCode(7)}`,
);
    ...
    try {
      if (input.startsWith("log")) {
        ...
        if (guild) {
          ...
          const subcommand = () =>
            rl.question(chalk.green(">>>") + " ", async (i) => {
              try {
                if (i.startsWith("leave")) {
                  ...
                  console.log(
                    `${guild.name} 的伺服器設定檔`,
                    Kami.database.GuildDatabase.get(guild.id),
                  );
                  ...
                  共同伺服器 ${chalk.gray("|")} ${Kami.guilds.cache
                    .filter((g) => g.members.cache.has(member.id))
                    .map((g) => `${g.name} ${chalk.gray(g.id)}`)
                    .join(`\n           ${chalk.gray("|")} `)}
              `);
...
rl._writeToOutput = function _writeToOutput(stringToWrite) {
  ...
  if (args) {
    ...
    rl.output.write(
      "\u001b[90m>>>\x1b[0m " + chalk.blackBright(args.join(" ")),
    );
  } else {
...
async function updateData() {
  try {
    const fetched_data = (
      await cwa.earthquake_report({ format: "json" }).catch(() => void 0)
    )?.records?.Earthquake;
    const fetched_data_s = (
      await cwa.earthquake_report_s({ format: "json" }).catch(() => void 0)
    )?.records?.Earthquake;
    ...
    const rts_list = await (
      await fetch(
        "https://raw.githubusercontent.com/ExpTechTW/API/master/Json/earthquake/station.json",
        { method: "GET" },
      )
    ).json();
    Kami.data.rts_stations = new Collection(
      Object.keys(rts_list).map((k) => [k, rts_list[k]]),
    );
  } catch (error) {
    console.error(error);
  }
